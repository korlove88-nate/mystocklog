...
const earlyClose=(year:number,month:number,day:number)=>{
  const date=new Date(Date.UTC(year,month-1,day)),iso=isoDate(date)
  const thanksgiving=nthWeekday(year,11,4,4),blackFriday=new Date(`${thanksgiving}T00:00:00Z`);blackFriday.setUTCDate(blackFriday.getUTCDate()+1)
  return iso===isoDate(blackFriday)||(iso===`${year}-12-24`&&isTradingDay(date))||(iso===`${year}-07-03`&&isTradingDay(date)&&!marketHolidays(year).has(iso))
}
...
export const getMarketHours = (now=new Date()):MarketHours => {
  ...
  const open=newYorkTimeToUtc(year,month,day,9,30),close=newYorkTimeToUtc(year,month,day,earlyClose(year,month,day)?13:16,0)
