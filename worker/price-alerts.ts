...
async function owner(request:Request,env:AlertBindings){
  ...
  const suppliedUrl=request.headers.get('x-supabase-url'),suppliedAnon=request.headers.get('x-supabase-anon-key'),safeSupabaseUrl=suppliedUrl&&/^https:\/\/[a-z0-9-]+\.supabase\.co\/?$/i.test(suppliedUrl)?suppliedUrl:null
  const supabaseUrl=env.SUPABASE_URL??safeSupabaseUrl,apiKey=env.SUPABASE_SERVICE_ROLE_KEY??suppliedAnon
  if(token&&supabaseUrl&&apiKey){const result=await fetch(`${supabaseUrl.replace(/\/$/,'')}/auth/v1/user`,{headers:{apikey:apiKey,Authorization:`Bearer ${token}`}});if(result.ok){const user=await result.json() as {id?:string};if(user.id)return{id:user.id,source:'supabase' as const}}}
  const sitesUser=request.headers.get('oai-authenticated-user-id')
  return sitesUser?{id:sitesUser,source:'sites' as const}:null
}
