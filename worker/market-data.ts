...
async function refreshSymbol(symbol:string,stored:StoredPayload|null,gf:GoogleFinanceRecord|undefined,bindings:MarketBindings):Promise<StoredPayload|null>{
  ...
  const oldFundamentals=stored?.fundamentals??null
  const fundamentals=quoteRaw||profileRaw?{ticker:symbol,companyName:profileRaw?.companyName??quoteRaw?.name??oldFundamentals?.companyName??null,sector:profileRaw?.sector??oldFundamentals?.sector??null,marketCap:numberOrNull(quoteRaw?.marketCap??profileRaw?.mktCap)??numberOrNull(oldFundamentals?.marketCap),pe:numberOrNull(quoteRaw?.pe)??numberOrNull(oldFundamentals?.pe),eps:numberOrNull(quoteRaw?.eps)??numberOrNull(oldFundamentals?.eps)}:oldFundamentals
  ...
  const fundamentalSource=(field:'marketCap'|'pe'|'eps'|'sector'|'company'):StockDataSources[typeof field]=>quoteRaw||profileRaw?'fmp':stored?.sources?.[field]??'stored'
