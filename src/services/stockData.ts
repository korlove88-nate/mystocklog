...
export type LiveQuoteState={status:'live'|'closed'|'failed'|'unavailable'|'delayed'|'ip_changed';updatedAt:string|null;error?:string}
export type CollectorStatus={collector_id:string;status:'NORMAL'|'IP_CHANGED'|'API_ERROR';previous_ip:string|null;current_ip:string|null;detected_at:string|null;last_success_at:string|null;last_error_code:string|null;last_error_message:string|null;updated_at:string}
export type StockDataResult = { stocks: StockSnapshot[]; mode: DataMode; marketDate: string | null; updatedAt: string | null; validation: ValidationRow[]; catalog:MarketCatalog; marketOverview:MarketOverviewItem[]; refresh:RefreshState; liveQuote:LiveQuoteState|null; collectorStatus:CollectorStatus|null }
...
class DashboardStockDataProvider implements StockDataProvider{
  ...
  async getStocks():Promise<StockDataResult>{
    ...
    return{stocks,mode:liveCount?'market':'fallback',marketDate:latestMarketDate,updatedAt:body.liveQuote?.updatedAt??body.refresh?.completedAt??latestUpdatedAt,validation,catalog,marketOverview:body.marketOverview??emptyMarketOverview,refresh:body.refresh??null,liveQuote:body.liveQuote??null,collectorStatus:body.collectorStatus??null}
  }
}
