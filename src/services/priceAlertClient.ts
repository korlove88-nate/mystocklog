...
const authFetch=async(service:StrategyNotesService,path:string,init:RequestInit={})=>{const token=await service.accessToken();if(!token)throw new Error('로그인이 필요합니다.');const result=await fetch(path,{...init,headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json',...init.headers}});const body=await result.json();if(!result.ok)throw new Error((body as {error?:string}).error??`요청 실패 ${result.status}`);return body}
