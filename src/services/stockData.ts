...
export type LiveQuoteState={status:'live'|'closed'|'failed'|'unavailable';updatedAt:string|null;error?:string}
export type StockDataResult = { stocks: StockSnapshot[]; mode: DataMode; marketDate: string | null; updatedAt: string | null; validation: ValidationRow[]; catalog:MarketCatalog; marketOverview:MarketOverviewItem[]; refresh:RefreshState; liveQuote:LiveQuoteState|null }
export interface StockDataProvider { getStocks(): Promise<StockDataResult> }

type DashboardPayload={payloads?:Record<string,import('./marketDataProvider').MarketDataPayload>;catalog?:MarketCatalog;marketOverview?:MarketOverviewItem[];refresh?:RefreshState;liveQuote?:LiveQuoteState}
class DashboardStockDataProvider implements StockDataProvider{
  constructor(private readonly liveQuote=false){}
  async getStocks():Promise<StockDataResult>{
    const result=await fetch(`/api/market-data?dashboard=1${this.liveQuote?'&liveQuote=1':''}`,{signal:AbortSignal.timeout(30_000)})
    ...
    return{stocks,mode:liveCount?'market':'fallback',marketDate:latestMarketDate,updatedAt:body.liveQuote?.updatedAt??body.refresh?.completedAt??latestUpdatedAt,validation,catalog,marketOverview:body.marketOverview??emptyMarketOverview,refresh:body.refresh??null,liveQuote:body.liveQuote??null}
  }
}
