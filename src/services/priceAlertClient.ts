...
export type AlertAuthSource='supabase'|'sites'
export type AlertDashboard={settings:AlertSetting[];events:AlertEvent[];pushSubscribed:boolean;authSource:AlertAuthSource;lastRun:{market_date?:string;completed_at?:string;status?:string;checked_count?:number;event_count?:number}|null;nextCheck:string}

const authFetch=async(service:StrategyNotesService|null,path:string,init:RequestInit={})=>{
  let token:string|null=null
  if(service)try{token=await service.accessToken()}catch{token=null}
  const identityHeaders:Record<string,string>=token?{Authorization:`Bearer ${token}`,'X-Supabase-URL':service!.url,'X-Supabase-Anon-Key':service!.anonKey}:{}
  const result=await fetch(path,{...init,credentials:'same-origin',headers:{'Content-Type':'application/json',...identityHeaders,...init.headers}})
  const body=await result.json()
  if(!result.ok)throw new Error((body as {error?:string}).error??(result.status===401?'MyStockLog 사이트 로그인 세션을 확인해 주세요.':`요청 실패 ${result.status}`))
  return body
}
export const loadAlerts=(service:StrategyNotesService|null)=>authFetch(service,'/api/alerts') as Promise<AlertDashboard>
export const alertAction=(service:StrategyNotesService|null,body:Record<string,unknown>)=>authFetch(service,'/api/alerts',{method:'POST',body:JSON.stringify(body)}) as Promise<AlertDashboard>
...
export type PushSubscribeResult={subscription:PushSubscription;endpoint:string;created:boolean;d1Saved:true;authSource:AlertAuthSource}

export async function subscribePush(service:StrategyNotesService|null,onStage?:(stage:PushSubscribeStage)=>void):Promise<PushSubscribeResult>{
  ...
  const saved=await authFetch(service,'/api/push/subscribe',{method:'POST',body:JSON.stringify(subscription.toJSON())}) as {ok?:boolean;authSource?:AlertAuthSource}
  if(!saved.ok)throw new Error('Push 구독을 서버에 저장하지 못했습니다.')
  onStage?.('d1-saved')
  return{subscription,endpoint:subscription.endpoint,created:!existing,d1Saved:true,authSource:saved.authSource??'sites'}
}
export const testPush=(service:StrategyNotesService|null)=>authFetch(service,'/api/push/test',{method:'POST',body:'{}'})
